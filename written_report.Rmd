...
hotels_total <- read_csv("data/hotels_reduced.csv")
dt <- sort(sample(nrow(hotels_total), nrow(hotels_total)*.7))
hotels <-hotels_total[dt,]
hotels_test <-hotels_total[-dt,]
```
...
```{r}
train_predictions <- model %>% predict(hotels)
test_predictions <- model %>% predict(hotels_test)

rmse(train_predictions, hotels$adr)
rmse(test_predictions, hotels_test$adr)
```
