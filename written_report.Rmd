...
#Appendix
```{r response-plot}
ggplot(data = hotels, aes(x = adr, fill = hotel)) +
  geom_histogram() + 
...
#Appendix
```{r}
ggplot(data = hotels, aes(x = meal, fill = hotel)) +
geom_bar() +
...
#Appendix
```{r}
ggplot(data = hotels, aes(x = reservation_status, fill = hotel)) +
geom_bar() +
...
```{r }
# Residual + QQ Plots
autoplot(selected_model, which = c(1,2), colour = "hotel")
```
