...
and find __, __, __, and _ to be the most important predictors. We find that 
bookings with _ and _ were associated with the largest increase in average daily hotel 
room rate, and bookings with _ and __ were associated with the largest decrease 
...
#Appendix
```{r}
ggplot(data = hotels, aes(x = reservation_status, fill = reservation_status)) +
geom_bar() +
...
geom_point(color="blue") +
labs(title = "Relationship Between Day in the Month When Hotel Rooms Are Booked
and Average Daily Rate",
  x = "Day in the Month",
  y = "Average Daily Rate") 
```
...
geom_point(color = "blue") +
labs(title = "Relationship Between Week in the Year When Hotel Rooms are Booked 
and Average Daily Rate",
  x = "Week in the Year",
  y = "Average Daily Rate") 
```
...
```{r}
model_interact <- lm(adr ~ hotel + is_canceled + lead_time + arrival_date_year + arrival_date_month + arrival_date_day_of_month + stays_in_week_nights + adults + children + meal + distribution_channel + is_repeated_guest + previous_bookings_not_canceled + days_in_waiting_list + total_of_special_requests + arrival_date_month*arrival_date_day_of_month, data = hotels)
...
```{r}
x0 <- tibble(hotel = "Resort Hotel", is_canceled = 0, lead_time = 90, arrival_date_year = "2016", arrival_date_month = "February",
             arrival_date_day_of_month = 26, stays_in_week_nights = 3, adults = 2, children =3, meal = "HB", distribution_channel = "Direct",
