...
Hotels are a critical component of the travel sector in the U.S., with an estimated 5.3 million guest rooms, and supporting 1 in 25 American jobs on average prior to 2020 [https://www.ahla.com/sites/default/files/2021_state_of_the_industry_0.pdf]. A 2018 report showed that 
approximately two thirds of Americans book their hotels directly through hotel websites [https://www.trekksoft.com/en/blog/65-travel-tourism-statistics-for-2019], and a 2019 analysis showed large
discrepancies in consumer travel cost based on how consumers chose to prioritize booking lodging compared to activities [https://globalnews.booking.com/bookingcom-reveals-key-findings-from-its-2019-sustainable-travel-report/]. 
...
We randomly sample 30% of our data to be used for training, and 30% for testing. 
...
```{r}
full_model <- lm(adr ~ ., data = hotel_train)
tidy(full_model) %>%
  kable(digits = 3)
```

```{r results = "hide"}
selected_model <- stats::step(full_model, scope=formula(full_model), direction="backward", k = log(nrow(hotel_train)))
```
...
```{r}
model <- lm(adr ~ adults + children + babies + hotel + meal + reservation_status, data = hotel_train)
```
...
our training and test sets, and find that on average, our predictions for the average
daily rate of hotels differ from the actual value by around $42-$45. 
